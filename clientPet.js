module.exports = function(){
    ...
    function getClientPet(res, mysql, context, complete){
        mysql.pool.query("SELECT IFNULL(clientPet.clientID, 'NULL') AS clientID, CONCAT(clients.fname, ' ', clients.lname) AS name, IFNULL(clientPet.petID, 'NULL') AS petID, pets.petName FROM clientPet INNER JOIN clients ON clients.clientID = clientPet.clientID INNER JOIN pets ON pets.petID = clientPet.petID", function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.end();
            }
